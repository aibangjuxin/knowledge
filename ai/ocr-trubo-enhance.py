...
def _get_runner():
    """
    返回事件循环 runner。
    Python 3.12+ 废弃了 set_event_loop_policy，
    正确做法是直接用 uvloop.run() 替代 asyncio.run()。
    缺包时静默降级为 asyncio.run。
    """
    try:
        import uvloop
...
def main():
    # 提速 #1: 获取事件循环 runner（uvloop.run 或 asyncio.run）
    _run = _get_runner()
    ...
    if args.batch:
        ...
        if not p.is_dir():
            print(f"❌  批量处理目录不存在：{p}")
            sys.exit(1)
        _run(process_batch(p, args.model))
    else:
        image_path = (
        ...
        _, raw_text, enhanced_text = _run(process_single(image_path, args))
