...
## GKE Gateway 深度解析与配置示例

根据您的需求，我们来详细分析如何配置 GKE Gateway 以实现将 `https://www.aibang.com/api-name-health1` 和 `https://www.aibang.com/api2-name-health2` 分别路由到不同 Namespace (`tenant-a` 和 `tenant-b`) 的后端服务。

此方案的核心是利用 Kubernetes Gateway API 的标准资源，实现一个强大且灵活的路由层。

### 核心流程

1.  **外部请求**: 客户端向 `https://www.aibang.com` 发起请求，该域名解析到 GKE Gateway 创建的外部负载均衡器 (GLB) 的静态 IP。
2.  **TLS 终止**: GKE Gateway 在 GLB 层利用您提供的证书 (`www-aibang-com-cert`) 对客户端的 HTTPS 请求进行 TLS 终止。
...
graph TD
    ...
    subgraph "GCP - aibang.com"
        A -- "HTTPS Request<br/>/api-name-health1/" --> B{"GKE Gateway (GLB)<br/>Static IP"}
        B -- "TLS Termination" --> C["Gateway Resource: 'external-http'"]
...
这是整个架构的入口点，定义了一个全局外部负载均衡器。它在 `default` 命名空间中创建，但可以被其他命名空间中的 `HTTPRoute` 引用。
  ...
  listeners:
    - name: https
      ...
      tls:
        mode: Terminate
        certificateRefs:
          - name: www-aibang-com-cert # 您需要预先创建这个 Secret
            kind: Secret
            group: ""
```
**说明**:
*   `gatewayClassName: gke-l7-global-external-managed`: 指定使用 GKE 托管的全局外部负载均衡器。
*   `protocol: HTTPS`, `port: 443`: 监听标准的 HTTPS 端口。
*   `allowedRoutes`: 允许所有命名空间的 `HTTPRoute` 附加到此 Gateway。
*   `certificateRefs`: 引用一个包含 `www.aibang.com` TLS 证书和私钥的 Kubernetes Secret。

---

#### 2. Tenant A 资源

为 `tenant-a` 创建独立的命名空间、路由、服务和应用部署。
...
**说明**:
*   **HTTPRoute**:
    *   `parentRefs`: 附加到 `default` 命名空间中的 `external-http` Gateway。
    *   `hostnames`: 仅匹配 `www.aibang.com` 的请求。
    *   `rules`: 匹配路径前缀为 `/api-name-health1/` 的请求，并将其转发到 `api-name-health1-svc`。
*   **Service**:
    *   `annotations: cloud.google.com/app-protocols: '{"https_port":"HTTPS"}'`: **这是关键**。此注解告知 GKE Gateway，与此服务后端 Pod 的通信必须使用 HTTPS 协议。
    *   `port: 8443`: 服务暴露 8443 端口。
*   **Deployment**:
    *   `containerPort: 8443`: 您的应用容器必须在此端口上监听 HTTPS 流量。
    *   `volumes` & `volumeMounts`: 从名为 `my-intra-gcp-uk-aibang-local-cert` 的 Secret 中挂载内部证书和私钥到 Pod 中，供应用使用。

---

#### 3. Tenant B 资源

为 `tenant-b` 创建类似的资源，仅修改名称和路径。
...
1.  **创建证书 Secret**:
    *   为外部域名 `www.aibang.com` 创建一个 TLS Secret `www-aibang-com-cert`。
...
2.  **应用 YAML**: 按顺序应用以上 YAML 文件。
    ...
    ```
3.  **获取 IP 地址**: GKE Gateway Controller 会自动创建一个外部负载均衡器。几分钟后，您可以通过以下命令获取其 IP 地址：
    ```bash
    kubectl get gateway external-http -n default -o jsonpath='{.status.addresses[0].value}'
    ```
4.  **配置 DNS**: 将 `www.aibang.com` 的 A 记录指向获取到的 IP 地址。

完成以上步骤后，您的 GKE Gateway 就配置好了，可以根据 URL 路径将流量安全地路由到不同命名空间中的 HTTPS 后端服务。
