...
新增 get_readiness_probe_urls() 函数：

静默版本，只返回 URL 列表，不输出信息
与 get_ingress_urls() 保持一致的行为
分别收集和测试两种 URL：

Ingress URLs：入口点测试
Readiness Probe URLs：健康检查端点测试
每种 URL 都有独立的收集、去重和测试流程
独立的测试和汇总：

分别测试 Ingress URLs 和 Readiness Probe URLs
每种测试都有独立的成功/失败计数
每种测试都有独立的结果汇总
综合汇总报告：

显示总体测试结果
分别显示 Ingress URLs 和 Readiness Probe URLs 的通过率
提供详细的统计信息
改进的输出格式：

清晰区分两种类型的 URL
使用不同的图标（🌐 for Ingress, 📋 for Readiness）
分段显示测试结果，便于理解
现在脚本会：
