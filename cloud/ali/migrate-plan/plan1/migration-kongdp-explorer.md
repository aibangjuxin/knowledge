...
*.source-team1.dev.aliyun.cloud.cn.aibang ==> 10.72.10.88
api01.source-team1.dev.aliyun.cloud.cn.aibang ==> 10.72.10.88
api02.source-team1.dev.aliyun.cloud.cn.aibang ==> 10.72.10.88
...
Ingress controller ==> SVC ==> RT
因为更换了工程所以域名会变更为
*.target-team2.dev.aliyun.cloud.cn.aibang ==> 10.72.11.99
api01.target-team2.dev.aliyun.cloud.cn.aibang ==> 10.72.11.99
api02.target-team2.dev.aliyun.cloud.cn.aibang ==> 10.72.11.99
...
```mermaid
flowchart TD
    A[Client] --> B[*.source-team1.dev.aliyun.cloud.cn.aibang<br/>10.72.10.88]
...
flowchart TD
    ...
    A --> C[Long Domain<br/>*.target-team2.dev.aliyun.cloud.cn.aibang<br/>10.72.11.99]
...
| 域名类型 | 域名                                       | IP 地址     | 用途                 |
| -------- | ------------------------------------------ | ----------- | -------------------- |
| 泛域名   | \*.source-team1.dev.aliyun.cloud.cn.aibang | 10.72.10.88 | A Cluster (原)       |
| 泛域名   | \*.target-team2.dev.aliyun.cloud.cn.aibang | 10.72.11.99 | B Cluster (新长域名) |
| 短域名   | dev-01.aliyun.cloud.cn.aibang              | 10.72.11.99 | B Cluster (新短域名) |
...
2.  **域名解析配置：**
    ...
    - 在 DNS 服务中，为 `*.target-team2.dev.aliyun.cloud.cn.aibang` 配置泛解析，指向 B Cluster 的 Ingress Controller IP (`10.72.11.99`)。
    - 为 `api01.target-team2.dev.aliyun.cloud.cn.aibang` 和 `api02.target-team2.dev.aliyun.cloud.cn.aibang` 配置 DNS A 记录，指向 `10.72.11.99`。
...
2.  **域名解析配置：**
    ...
    - 在 DNS 服务中，为 `dev-01.aliyun.cloud.cn.aibang` 配置 DNS A 记录，指向 B Cluster 的 Ingress Controller IP (`10.72.11.99`)。
...
- 泛解析域名：\*.source-team1.dev.aliyun.cloud.cn.aibang 指向 IP 10.72.10.88
- 示例：
...
- 新域名：\*.target-team2.dev.aliyun.cloud.cn.aibang 指向 IP 10.72.11.99。
- 示例：
...
- 阿里云 DNS（AliDNS）需更新泛解析记录指向新 IP 10.72.11.99。
...
在 A 集群的现有架构中，外部流量通过阿里云 DNS 的泛解析记录（例如 _.source-team1.dev.aliyun.cloud.cn.aibang）被导向至其 Ingress 控制器的负载均衡器 IP (10.72.10.88) 。该 Ingress 控制器实际上是一个由 CCC 团队管理的 Kong DP，它作为 API 网关承担着统一的流量入口职责 。Kong DP 依据接收到的请求主机名（Host Header）和路径（Path）等路由规则，将请求精准地转发至后端的 Service，最终由服务背后的 Pod（RT）进行处理 。
...
- DNS 层面的流量控制：
    利用阿里云 DNS（Authoritative DNS）的加权路由功能，可以对指向旧域名 \*.source-team1... 的请求进行精细化的流量分配 。在迁移准备阶段，可以为该域名配置两条 A 记录：一条指向 A 集群的负载均衡器 IP (10.72.10.88)，另一条指向 B 集群的新负载均衡器 IP (10.72.11.99)。通过调整这两条记录的权重（例如，99%指向 A，1%指向 B），可以实现流量的渐进式灰度切换 。阿里云 DNS 支持通配符解析和智能解析，这与 Azure Traffic Manager 和 AWS Route 53 的功能类似，为跨集群迁移提供了原生、强大的支持 。
- Ingress 层面的多域名监听：
...
- DNS 预配置：
    - 在阿里云 DNS 中，为新的泛解析域名 \*.target-team2.dev.aliyun.cloud.cn.aibang 创建 A 记录，指向 B 集群的 ALB 公网 IP (10.72.11.99)。为了加快后续切换的生效速度，建议将该记录的 TTL 值设置为一个较小的值（例如 60 秒），但要意识到 DNS 传播仍然存在延迟 。
        3.2 阶段二：渐进式流量切换与验证
- 内部验证：
...
- 最终流量切换与旧集群清理：
    ...
    - 一旦所有旧域名流量都已清零，并且短域名模式稳定运行，可以逐步清理 A 集群的资源，包括 Kubernetes 集群、ALB 和 EIP 等，释放占用的资源。
        ...
        | 阶段 | 域名 | 记录类型 | 记录值 | 备注 |
        |---|---|---|---|---|
        | 阶段一：准备 | _.source-team1... | A | 10.72.10.88 | 指向 A 集群，业务正常运行 |
        | | _.target-team2... | A | 10.72.11.99 | 指向 B 集群，用于内部验证 |
        | 阶段二：灰度 | _.source-team1... | 加权 A | 10.72.10.88 (99%)<br>10.72.11.99 (1%) | 流量逐渐从 A 集群切换至 B 集群 |
        | | _.target-team2... | A | 10.72.11.99 | 新域名可正常访问 |
        | 阶段三：最终 | _.target-team2... | A | 10.72.11.99 | 备用长期域名，可选 |
        | | dev-01... | A | 10.72.11.99 | 最终短域名入口 |
