...
scale hpa 
```
kubectl scale deployment name-version-master-1-1-deployment --replicas=2 -n abcoe

8886666666$ kubectl get pod -n abcoe|grep  name-version-master
...
kubectl get pods -o json -n abcoe |/home/lex/jq '.items[] | {"name": .metadata.name, "age": .metadata.creationTimestamp}'

8886666666$ kubectl scale deployment name-version-master-1-1-deployment --replicas=2 -n abcoe
...
8886666666$ kubectl get pods --sort-by=.metadata.creationTimestamp -l app=name-version-master-1-1 -o jsonpath='{range .items[0]}{.metadata.name}{end}' -n abcoe
name-version-master-1-1-deployment-7 -n abcoe|grep  name-version-master
name-version-master-1-1-deployment-7bcdf5f49-jqjk9           1/1     Running            0                15h
name-version-master-1-1-deployment-7bcdf5f49-pm8vb           1/1     Running            0                24s
8886666666$ kubectl get pod -n abcoe --show-labels|grep name-version-master
...
8886666666$ kubectl get pods -l app=name-version-master-1-1 -o json -n abcoe |/home/lex/jq '.items[] | {"name": .metadata.name, "age": .metadata.creationTimestamp}'
{
...
kubectl scale --current-replicas=1 --replicas=2 deployment name-version-master-1-1-deployment -n abcoe

--selector 可以加 但是是针对的 deployment
https://www.airplane.dev/blog/kubectl-scale

8886666666$ kubectl get hpa name-version-master-1-1-deployment-hpa -n abcoe -o yaml
