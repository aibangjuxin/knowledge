...
但是，**在应用公式之前，必须通过容忍度检查**。

### 1.1 容忍度 (Tolerance) 机制

默认的容忍度为 **0.1 (10%)**。

- **扩容条件**：`Current / Desired > 1.1` (即指标超出目标值 10% 以上)
- **缩容条件**：`Current / Desired < 0.9` (即指标低于目标值 10% 以上)
- **静默区间**： `0.9 <= Ratio <= 1.1` (此时 HPA **不进行任何操作**)
...
由于 10% 的容忍度，只有当 CPU 利用率超过 `750% * 1.1 = 825%` 时，才会触发扩容。
**750% ~ 825% 之间尽管未达标，但属于容忍范围内，HPA 不会扩容。**

### 3.2. 从 1 个副本扩容到 2 个副本

- **触发阈值**：CPU > **825%**
- **计算逻辑**（假设 CPU 为 826%）：
    1.  **比率 (Ratio)**: $826 / 750 \approx 1.101$
    2.  **容忍度检查**: $1.101 > 1.1$ (超过10%容忍度，**触发扩容**)
    3.  **期望副本数**:
...
- **对比错误理解**：如果 CPU 是 760%，虽然大于 750%，但偏差仅 1.3%，HPA 会忽略忽略。
...
- **结论**：并不是只要超过 750% 就扩容，必须超过 825% 才会从 2 变 3。
...
- **容忍度约束**: $R < 0.9$
- **最终阈值**: $R \le 0.75$ 即 CPU $\le 750\% \times 0.75 = \mathbf{562.5\%}$
...
- **最终阈值**: CPU $\le 750\% \times 0.666 = \mathbf{500\%}$
...
- **最终阈值**: CPU $\le 750\% \times 0.5 = \mathbf{375\%}$
...
- **触发条件**：内存 > **88%** (81% 不会触发！)
- **你的案例 (81%/80%)**：
    - 比率 1.0125。偏差 1.25% < 10%。**不扩容**。
- **验证 (利用率 89%)**:
- 比率 $89/80 = 1.1125$。偏差 11.25% > 10%。**扩容**。
...
| 副本变化 | 关键约束因素 | 目标 750% (CPU) 触发值 | 目标 80% (Memory) 触发值 | 目标 90% (Memory) 触发值 |
| :--- | :--- | :--- | :--- | :--- |
| **扩容 (+1)** | **Tolerance (Ratio > 1.1)** | **> 825%** | **> 88%** | **> 99%** |
| **缩容 (4->3)** | Ceil (Ratio $\le$ 0.75) | $\le$ 562.5% | $\le$ 60% | $\le$ 67.5% |
| **缩容 (3->2)** | Ceil (Ratio $\le$ 0.66) | $\le$ 500% | $\le$ 53.3% | $\le$ 60% |
| **缩容 (2->1)** | Ceil (Ratio $\le$ 0.50) | $\le$ 375% | $\le$ 40% | $\le$ 45% |
