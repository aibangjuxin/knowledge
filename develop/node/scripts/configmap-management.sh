#!/bin/bash
# 平台 ConfigMap 管理脚本 - Node.js 版本
# 用于创建和管理 Java 和 Node.js 的 ConfigMap
...
echo -e "${GREEN}=== 平台 ConfigMap 管理工具（Node.js）===${NC}"
echo ""
...
create_nodejs_configmap() {
    echo -e "${YELLOW}创建 Node.js ConfigMap...${NC}"
...
EOF
    ...
    echo -e "${GREEN}✓ Node.js ConfigMap 创建完成${NC}"
    echo "  - Namespace: ${NAMESPACE}"
    echo "  - Name: mycoat-common-nodejs-conf"
    echo ""
}
...
view_configmaps() {
    ...
    echo ""
    echo -e "${BLUE}Node.js ConfigMap:${NC}"
    if kubectl get configmap mycoat-common-nodejs-conf -n ${NAMESPACE} >/dev/null 2>&1; then
        kubectl get configmap mycoat-common-nodejs-conf -n ${NAMESPACE}
        echo ""
        echo "配置内容:"
        kubectl get configmap mycoat-common-nodejs-conf -n ${NAMESPACE} -o jsonpath='{.data.server-conf\.properties}'
        echo ""
    else
        echo -e "${RED}✗ 不存在${NC}"
    fi
...
delete_configmaps() {
    ...
    read -p "删除 Node.js ConfigMap? (y/N) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        kubectl delete configmap mycoat-common-nodejs-conf -n ${NAMESPACE} 2>/dev/null && \
            echo -e "${GREEN}✓ Node.js ConfigMap 已删除${NC}" || \
            echo -e "${RED}✗ Node.js ConfigMap 不存在或删除失败${NC}"
    fi
    echo ""
}
