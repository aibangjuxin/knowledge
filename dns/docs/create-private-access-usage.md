# create-private-access.sh 使用文档

## 功能概述

这个脚本用于从现有的 `private-access` DNS Zone 导出记录，并创建一个新的环境特定的 Zone（格式：`{env}-{region}-private-access`），同时保持相同的网络绑定。

## 核心功能

1. **环境配置管理**：通过预定义的环境配置自动解析项目、区域和网络信息
2. **Zone 列表展示**：列出当前项目的所有 DNS Zones 及其网络绑定
3. **记录导出**：从源 Zone 导出所有 DNS 记录到 JSON 文件
4. **网络绑定获取**：自动获取源 Zone 绑定的网络
5. **Zone 创建**：创建新的 Zone 并绑定相同的网络
6. **记录导入**：将导出的记录导入到新 Zone
7. **对比验证**：对比源 Zone 和目标 Zone 的记录数量
...
```bash
./create-private-access.sh -e dev-cn
```

### 指定自定义源 Zone

```bash
./create-private-access.sh -e lex-in -s custom-zone
```

### 查看帮助

```bash
./create-private-access.sh -h
```
...
| 参数 | 说明 | 必需 | 默认值 |
|------|------|------|--------|
| `-e` | 环境标识（格式：{env}-{region}） | 是 | - |
| `-s` | 源 Zone 名称 | 否 | private-access |
| `-h` | 显示帮助信息 | 否 | - |
...
- `dev-cn`: aibang-teng-sit-api-dev 项目
- `lex-in`: aibang-teng-sit-kongs-dev 项目

每个环境包含：
- `project`: GCP 项目 ID
- `region`: 区域
- `private_network`: 私有网络名称
...
========================================
环境标识: dev-cn
项目 ID: aibang-teng-sit-api-dev
区域: europe-west2
私有网络: aibang-teng-sit-api-dev-cinternal-vpc3
源 Zone: private-access
目标 Zone: dev-cn-private-access
```
...
========================================
┌─────────────────┬──────────────┬─────────────────────┬──────────────┐
│ DNS_NAME        │ CREATION_TIME│ NAME                │ NETWORKS     │
├─────────────────┼──────────────┼─────────────────────┼──────────────┤
│ example.com.    │ 2024-01-01   │ private-access      │ vpc-network  │
└─────────────────┴──────────────┴─────────────────────┴──────────────┘
```

### 3. 记录导出
```
导出 Zone 'private-access' 的所有记录...
✓ 成功导出 25 条记录到: /tmp/private-access-records-20260212-111715.json
...
对比 Zone 记录
========================================
源 Zone (private-access) 记录数: 22
目标 Zone (dev-cn-private-access) 记录数: 22
✓ 记录数量一致
```

## 注意事项

1. **网络绑定**：新 Zone 会自动绑定源 Zone 相同的网络
2. **记录过滤**：NS 和 SOA 记录会被自动跳过（这些是 Zone 自动生成的）
3. **重复记录**：如果记录已存在，会自动跳过
4. **导出文件**：记录会导出到 `/tmp` 目录，文件名包含时间戳
5. **Zone 覆盖**：如果目标 Zone 已存在，脚本会询问是否删除重建
...
这将：
1. 从 `private-access` Zone 导出所有记录
2. 创建 `dev-cn-private-access` Zone
3. 绑定 `aibang-teng-sit-api-dev-cinternal-vpc3` 网络
4. 导入所有记录

### 场景 2: 从自定义源 Zone 迁移

```bash
./create-private-access.sh -e lex-in -s my-custom-zone
```
