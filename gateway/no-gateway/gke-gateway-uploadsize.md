...
这样 IIP 档掉异常大的恶意请求，G Kong/Runtime 做精确的业务限制。
...
是的，当前基于 Google Kubernetes Engine Gateway API 的原生 GKE Gateway（即 Google Cloud Gateway Controller / Global External Application Load Balancer）没有像 Nginx Ingress 那样直接提供 request body size limit 配置项。
...
- 也没有 Gateway API 原生字段：

  ```yaml
  maxRequestBodySize
  ```

所以你不能直接在以下资源中配置上传文件大小限制：
...
GKE Gateway 背后使用的是 Google Cloud Load Balancer（GLB / Envoy-based data plane）。

而 Google 的 Gateway API 实现更偏向 L7 Routing、TLS、Traffic Policy、Cloud Armor、Header rewrite、Timeout、Retry 等能力。

但并没有暴露 Envoy 的 buffer filter、body limit、request payload control 等能力。

---

### 当前 GKE Gateway 支持的常见能力
...
| Express.js    | `body-parser limit`    |
| Kong          | `client_max_body_size` |
| Istio EnvoyFilter | `buffer limit`    |
...
### 方案 2 — 在 Gateway 后面再挂一层 Nginx/Kong
...
这是很多企业现在真实在 GKE 上的做法。尤其在 API Platform、文件上传、OpenAPI Gateway、AI 上传、图片系统等场景都会这样做。
...
其实已经天然具备这个能力。所以你们的平台：实际应该已经能做 request body limit。只是——不是 GKE Gateway 提供的，而是 Kong/Nginx 提供的。

---

### 方案 3 — Cloud Armor（部分替代）

Google Cloud Armor 可以限制 request header、WAF、rate limit、geo/ip、body inspection（有限）。

但它不能真正限制上传文件大小：

- Cloud Armor 只 inspect body 前几个 KB
- 不是 upload quota system
- 不是 body buffer controller

所以不能替代 `client_max_body_size`。

---

## 附录 C：Gateway API 为什么没有这个字段？

这是 Gateway API 社区本身的问题。标准 Gateway API（Gateway、HTTPRoute、BackendPolicy）本身并未标准化 `maxRequestBodySize`，因为不同实现差异很大：

| 实现           | Data Plane    |
| ------------ | ------------- |
| Nginx Gateway  | Nginx          |
| Istio Gateway  | Envoy          |
| Kong Gateway   | Nginx/OpenResty |
| GKE Gateway    | Google Envoy    |
| AWS Gateway    | ALB            |

很难统一，所以目前属于 `implementation-specific feature`。

---

### 如果你一定要在 GKE Gateway 层做

目前没有官方原生支持，只能：

**方法 1 — 换用支持 body limit 的 Gateway：**

- Nginx Gateway Fabric
- Kong Gateway
- Istio Gateway

**方法 2 — 自定义 Envoy Filter（GKE Gateway 不开放）**

但 GKE Gateway 不允许你直接控制底层 Envoy config，因此不可行。
...
Google Gateway 很强——全球 LB、TLS、Anycast、Cloud Armor、HA，但 API 精细治理能力不足。所以大家通常：

- Google Gateway 负责入口
- Kong/Istio 负责 API policy

包括 request size、JWT、WAF、rate limit、body transform、authz、retry、circuit breaker 等。
...
但这些都不是用户可配置的 body limit。

---

## 一句话总结

GKE Gateway / Google Gateway Controller 当前原生不支持 request body size limit。上传文件大小限制通常需要通过后端应用、Nginx、Kong、Istio 来实现，而不是通过 Gateway API 本身。
