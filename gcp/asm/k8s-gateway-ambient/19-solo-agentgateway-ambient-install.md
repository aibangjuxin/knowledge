- [q](#q)
- [Solo Istio 1.30.3 Ambient + agentgateway + KongDP 在 GKE 的落地推导](#solo-istio-1303-ambient--agentgateway--kongdp-在-gke-的落地推导)
  - [1. 问题分析：这 4 个镜像能推导出什么，不能推导出什么](#1-问题分析这-4-个镜像能推导出什么不能推导出什么)
    - [1.1 镜像 → 组件 → Helm Chart 映射](#11-镜像--组件--helm-chart-映射)
    - [1.2 缺失清单（需要补充到你们的内部镜像仓库）](#12-缺失清单需要补充到你们的内部镜像仓库)
    - [1.3 Tag 需要核对](#13-tag-需要核对)
  - [2. GatewayClass 的三条路线（这是你问题的核心）](#2-gatewayclass-的三条路线这是你问题的核心)
    - [2.1 是否需要保留 `istio-waypoint`？—— 需要，而且应该作为默认](#21-是否需要保留-istio-waypoint-需要而且应该作为默认)
  - [3. 整体流量架构](#3-整体流量架构)
  - [4. 安装步骤](#4-安装步骤)
    - [4.1 环境变量与前置检查](#41-环境变量与前置检查)
    - [4.2 Ambient 核心组件（顺序不可颠倒）](#42-ambient-核心组件顺序不可颠倒)
    - [4.3 `values-istiod-ambient.yaml`](#43-values-istiod-ambientyaml)
    - [4.4 `values-cni-gke.yaml`](#44-values-cni-gkeyaml)
    - [4.5 `values-ztunnel.yaml`](#45-values-ztunnelyaml)
  - [5. 关键配置文件](#5-关键配置文件)
    - [5.1 命名空间纳管（渐进式）](#51-命名空间纳管渐进式)
    - [5.2 Ingress Gateway + ListenerSet](#52-ingress-gateway--listenerset)
    - [5.3 HTTPRoute：强制经过 KongDP](#53-httproute强制经过-kongdp)
    - [5.4 waypoint（`istio-waypoint`）](#54-waypointistio-waypoint)
    - [5.5 waypoint 上的 L7 Filter（Header + Body Size）](#55-waypoint-上的-l7-filterheader--body-size)
    - [5.6 身份与授权（SPIFFE）](#56-身份与授权spiffe)
  - [6. KongDP 融入 Ambient 的专项注意事项](#6-kongdp-融入-ambient-的专项注意事项)
    - [6.1 端口排除](#61-端口排除)
    - [6.2 Host 头必须透传](#62-host-头必须透传)
    - [6.3 ztunnel 不解析 Host，走 Service VIP](#63-ztunnel-不解析-host走-service-vip)
    - [6.4 双 L7 hop 的性能账](#64-双-l7-hop-的性能账)
  - [7. 验证与排错](#7-验证与排错)
    - [常见故障对照表](#常见故障对照表)
  - [8. 待确认清单（建议先拉齐这几项再动手）](#8-待确认清单建议先拉齐这几项再动手)
- [Solo agentGateway + Ambient Mesh 在 GKE 上的安装路径(独立方案)](#solo-agentgateway--ambient-mesh-在-gke-上的安装路径独立方案)
  - [0. 文档定位](#0-文档定位)
    - [0.1 与已有文档的关系](#01-与已有文档的关系)
    - [0.2 起点(已有)](#02-起点已有)
    - [0.3 本次新增(本文回答)](#03-本次新增本文回答)
  - [1. 核心概念:agentGateway 不是一个东西,是 3 个独立产品](#1-核心概念agentgateway-不是一个东西是-3-个独立产品)
    - [1.1 简化 vs 严格定义](#11-简化-vs-严格定义)
    - [1.2 三个 GatewayClass 角色分工(关键澄清)](#12-三个-gatewayclass-角色分工关键澄清)
    - [1.3 为什么需要 agentGateway(而不是只用社区 `istio-waypoint`)](#13-为什么需要-agentgateway而不是只用社区-istio-waypoint)
  - [2. 安装路径总览(7 步)](#2-安装路径总览7-步)
    - [2.1 与已有文档的边界](#21-与已有文档的边界)
  - [3. Step 1-3:前置(快速复述,见 `02-` 详述)](#3-step-1-3前置快速复述见-02--详述)
    - [3.1 镜像仓库与 tag](#31-镜像仓库与-tag)
    - [3.2 Helm 安装 4 个组件(从 `02-` 复述,只列命令骨架)](#32-helm-安装-4-个组件从-02--复述只列命令骨架)
  - [4. Step 4:安装 Solo Enterprise for agentgateway 控制面](#4-step-4安装-solo-enterprise-for-agentgateway-控制面)
    - [4.1 Helm 仓库与 chart 名](#41-helm-仓库与-chart-名)
    - [4.2 安装 CRD chart](#42-安装-crd-chart)
    - [4.3 安装 control plane chart](#43-安装-control-plane-chart)
    - [4.4 values.yaml 完整示例(生产推荐配置)](#44-valuesyaml-完整示例生产推荐配置)
    - [4.5 验证安装](#45-验证安装)
  - [5. Step 5:创建 3 个 GatewayClass 对应的 Gateway 资源](#5-step-5创建-3-个-gatewayclass-对应的-gateway-资源)
    - [5.1 Gateway A:南北向入口 Gateway(`enterprise-agentgateway`)](#51-gateway-a南北向入口-gatewayenterprise-agentgateway)
    - [5.2 Gateway B:ambient L7 waypoint(`enterprise-agentgateway-waypoint`)](#52-gateway-bambient-l7-waypointenterprise-agentgateway-waypoint)
    - [5.3 Gateway C(可选):社区 `istio-waypoint`](#53-gateway-c可选社区-istio-waypoint)
    - [5.4 三 Gateway 何时共存 vs 互斥](#54-三-gateway-何时共存-vs-互斥)
    - [5.5 业务 ns enroll waypoint(声明要走 waypoint)](#55-业务-ns-enroll-waypoint声明要走-waypoint)
  - [6. Step 6:Kong DP 整合(关键新需求)](#6-step-6kong-dp-整合关键新需求)
    - [6.1 业务场景](#61-业务场景)
    - [6.2 Kong DP 必须放哪里?**mesh 外前置网关**](#62-kong-dp-必须放哪里mesh-外前置网关)
    - [6.3 完整流量路径(Kong 前置模式)](#63-完整流量路径kong-前置模式)
    - [6.4 用 DestinationRule SPIFFE 验证 Kong 身份](#64-用-destinationrule-spiffe-验证-kong-身份)
      - [6.4.1 Kong DP 在 mesh 内的边界处理](#641-kong-dp-在-mesh-内的边界处理)
      - [6.4.2 DestinationRule 配 SPIFFE 用于 mesh 内 Kong → Runtime 段](#642-destinationrule-配-spiffe-用于-mesh-内-kong--runtime-段)
    - [6.5 HTTPRoute 把 Gateway 路由到 Kong](#65-httproute-把-gateway-路由到-kong)
    - [6.6 引通(内部用户)怎么走 Kong](#66-引通内部用户怎么走-kong)
  - [7. Step 6.5:L7 策略(EnterpriseAgentgatewayPolicy)](#7-step-65l7-策略enterpriseagentgatewaypolicy)
    - [7.1 File upload 大小限制(CEL-based)](#71-file-upload-大小限制cel-based)
    - [7.2 Header 过滤(强制必备头)](#72-header-过滤强制必备头)
    - [7.3 SPIFFE-based 鉴权(workload identity)](#73-spiffe-based-鉴权workload-identity)
  - [8. 验证清单(全链路)](#8-验证清单全链路)
    - [8.1 控制面验证](#81-控制面验证)
    - [8.2 数据面验证](#82-数据面验证)
    - [8.3 端到端验证](#83-端到端验证)
  - [9. 决策表(快速选型参考)](#9-决策表快速选型参考)
    - [9.1 「要不要装 agentGateway 控制面?」](#91-要不要装-agentgateway-控制面)
    - [9.2 「waypoint 用哪个 GatewayClass?」](#92-waypoint-用哪个-gatewayclass)
    - [9.3 「Kong 放哪里?」](#93-kong-放哪里)
  - [10. 已知坑 / Pitfalls](#10-已知坑--pitfalls)
    - [10.1 GatewayClass 冲突](#101-gatewayclass-冲突)
    - [10.2 License 与 Image 关系](#102-license-与-image-关系)
    - [10.3 agentGateway 控制面 vs 数据面 License](#103-agentgateway-控制面-vs-数据面-license)
    - [10.4 ztunnel SPIFFE 不能跨出 mesh](#104-ztunnel-spiffe-不能跨出-mesh)
    - [10.5 ambient ↔ sidecar 共存期间](#105-ambient--sidecar-共存期间)
    - [10.6 端口 15008 NetworkPolicy](#106-端口-15008-networkpolicy)
    - [10.7 Kong → Runtime 之间无 mTLS](#107-kong--runtime-之间无-mtls)
  - [11. 权威证据(引用来源)](#11-权威证据引用来源)
    - [11.1 Solo agentGateway 官方](#111-solo-agentgateway-官方)
    - [11.2 Solo Istio 1.30 ambient 官方](#112-solo-istio-130-ambient-官方)
    - [11.3 Istio ambient 官方](#113-istio-ambient-官方)
    - [11.4 SPIRE / SPIFFE 官方](#114-spire--spiffe-官方)
    - [11.5 镜像仓库](#115-镜像仓库)
  - [12. 与已有文档的差异声明](#12-与已有文档的差异声明)
  - [13. 决策记录(本地 ADR-LOCAL-002 草稿)](#13-决策记录本地-adr-local-002-草稿)
    - [决策](#决策)
    - [备选方案](#备选方案)
    - [后果](#后果)
    - [回滚路径](#回滚路径)
