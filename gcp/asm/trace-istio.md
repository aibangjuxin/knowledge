...
* 处理： * 如果 `VirtualService` 跨 Namespace，需要解析其 FQDN 全称。
...
总结建议的入参设计
为了让脚本最灵活，建议入参设计为： `./trace-istio.sh --url https://api.example.com/v1/health`
脚本内部逻辑：
...
需要你帮我写这样一个脚本shell脚本 我有bash jq 当然目前我是这个cluster的管理员，我可以列出所有的资源。是想要最终输出这样一个Mapping关系，我知道哪里去找对应的资源，这样的话有利于我去提bug或者排查问题。
...
graph TD
    ...
    B --> C["STEP 1: 遍历所有 Gateway\n精确匹配 > 通配符 *"]
    C --> D["获取 Gateway NS/Name\n+ IngressGateway Pods"]
    D --> E["STEP 2: 查找 VirtualService\ngateways 字段匹配\n+ host 匹配\n+ path 前缀过滤"]
    E --> F["STEP 3: 提取 Destination\n补全 FQDN\n关联 DestinationRule\nSubset/TLS 策略"]
    F --> G["STEP 4: 查找 K8s Service\nClusterIP / Ports\nEndpoints / Pods\nSubset label 验证"]
