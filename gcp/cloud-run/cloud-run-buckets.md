...
1. 每当该文件更新（例如合并 PR），就通过一个 Cloud Run Job 自动运行

1. Cloud Run Job 拉取 GitHub 文件，将其同步到 GCS 中指定路径，如：
