...
- **成本与扩展：**
  - Cloud Run 按请求数量和��器运行时间计费。由于 WebSocket 连接会保持容器活跃，建议监控使用情况以控制成本。
  - 可以设置实例数和并发限制，优化资源使用。
- **权限与安全：**
...
Google Cloud 将代码的运行方式主要分为三种资源类型：服务（Service）、作业（Job）和工作池（Worker Pool）3。根据您的需求——执行基础设施变更、运行定时任务以及通过事件触发发布流程——我��将重点关注服务和作业这两种资源。
...
- **生命周期**：服务的设计是持���运行以随时处理传入的请求。其最显著的特性是能够根据流量自动扩缩容，甚至在没有流量时**缩容至零（scale to zero）**，从而极大地节约成本 2。服务的容器生命周期包括三个主要阶段：
...
综上所述，使用 WebSocket 模式会在一个为无状态、请求驱动而设计的 serverless 平台上，强行引入一个脆弱的、有状态的依赖。更稳健和符合 GCP 设计哲学的模式是使用**事件驱动**的机���来触发短暂的、一次性的**Cloud Run 作业**。我们将在第三章详细阐述这个更优的模式。
...
```dockerfile
# Layer 1: 基础镜像
# 选择一���精简且受信任的基础镜像，例如 Ubuntu Slim 或 Distroless
FROM ubuntu:22.04
...
您的代理需要知道从哪里下载 Bash 构件以及相关的发布参数。有两种推荐���方式来传递这些动态信息。
...
您的“Infra change”任务���能包含许多可以独立执行的子任务（例如，同时更新 10 个不同的防火墙规则，或在 5 个 GCS 存储桶上设置相同的权限）。传统的脚本会按顺序串行执行，效率低下。
...
这种模式将一个原本可能耗时数十分钟的串行过程，转变为一个高效的、仅需几分钟即可完成的并行过程。这不仅是技术上的��化，更是对您整个自动化平台能力的一次重大提升。
...
- **模式错配**：WebSocket 主要用于客户端与服务器之间的双向实时通信，例如聊天应用或实时仪表盘 24。而您的需求是服务到服务（或系统到服务）的异��命令分发，这正是消息队列和发布/订阅系统的专长。
...
- **灵��性与可扩展性**：您的控制平面（CP）的职责被简化为仅仅是向一个 Pub/Sub 主题发布一条消息。它完全不需要知道部署代理的存在形式、部署位置，甚至它是否正在运行。未来，您可以轻易地增加新的消费者（例如，一个用于审计的日志服务），或者替换掉当前的部署代理，而无需对 CP 做任何修改 31。
...
3.  **控制平面（CP）决策**：在流水线的最后阶段，一个脚本（作为 CP）被执行。它会构建一个描述发布任务的 JSON 消��，例如：
...
1.  **调用者服务账号（Invoker SA）**：
    - **使用者**：这个 SA 被您的控制��面（如 Cloud Build 流水线）或 Cloud Scheduler 作业所使用。
...
2.  **授予访问权限**：将 `roles/secretmanager.secretAccessor` 角色授予您的**作业运行时服务账号（Runtime SA）**，作用域是���需要访问的**特定密钥**。这再次遵循了最小权限原则。
...
3.  **配置网络出入口**：
    ...
    - **出口（Egress）**：配置 Cloud Run 作业的出口流量，使其通过 **VPC 连接器**或**直接 VPC 出口**路由到您的 VPC 网络。然后，您可以在 VPC 网络上应用防火墙规则，精确控制代理能够访问哪些外部地址，例如，只允许访问公司内部的 Nexus 服务器地址，而阻止所��其他出站流量 40。
...
首先需要明确一点：此处的 CI/CD 流水线是用于**更新部署代理本身**的，而不是用它来部署您的其他应用程序。一��稳定、可靠的自动化工具是所有后续工作的基础。
...
在发布失败时，您需要快速定位问题，而不是在海量的日志中手动��选。Cloud Logging 强大的查询语言（Logging Query Language）可以帮助您实现这一点。
...
2.  **创建策略（Create Policy）**：
    ...
    - **配置触发器（Configure trigger）**：
      ...
      - **阈值**：`0`。
      - **高级选项 -> 重新测���窗口**：设置为一个合适的时间，例如 `5 minutes`。
...
- **强制执行双层 IAM 模型 (Enforce a Two-Level IAM Model)**：严格区分**调用作业**的权限和**作业运行时**的权限。这是保障系统安全的核心原则，能有效限制潜在攻击的爆炸��径，实现真正的最小权限。
...
```mermaid
flowchart TD
    A[CP平台发送发布事件] --> B[Agent容��在Cloud Run Job中启动]
