...
**行为**：
- GKE 需要**显式启用** Gateway API 控制器（`--gateway-api=standard`），不是随集群默认安装，也不通过 Kubernetes feature gate 管控
- 支持 GKE 标准集群和 Autopilot 集群（Kubernetes 1.26 及以上）
- 这些 CRD **不随 GKE 版本升级而变化**（Gateway API CRD 有自己的版本周期，独立于 Kubernetes 版本）
- 创建时间 `2025-10-29` 的表示首次启用 Gateway API addon 时安装，之后未变
...
**行为**：
- 由 Istio 安装时一次性创建，istiod 运行时**不会**自动 re-apply CRD
- 创建时间 `2026-05-29 02:11:12Z`，与 Istio 安装时间一致
- **Istio 版本升级时**：需通过 `istioctl upgrade` 或 `helm upgrade istio-base` 显式更新 CRD；istiod 本身升级**不会**自动更新 CRD schema
- **风险场景**：Istio 升级时如未更新 CRD，可能导致新版 istiod 所需字段缺失；已有的 CR 实例在 CRD schema 变更后可能需要更新
...
`listenersets.gateway.networking.k8s.io` 是 Kubernetes Gateway API 中的 CRD，用于为一组已存在的 Gateway 附加额外的监听器配置。它最早以实验版 `XListenerSet`（API group: `gateway.networking.x-k8s.io/v1alpha1`）形式在 **Gateway API v1.3.0** 引入，在 **Gateway API v1.5.0**（2026年初）正式升入标准 channel（`gateway.networking.k8s.io/v1`）。从 GitHub 源码（`apis/v1/listenerset_types.go`）可以看到：
...
Gateway API v1.5.0 Standard channel CRD 官方安装命令（来源于 GitHub Release 页面）：
