...
这里补充一个更准确的结论：GCP **Internal HTTP(S) Load Balancer** 确实可以在 URL Map 上配置 `retryPolicy`，但 **Global External / Regional External Application Load Balancer 也同样支持 URL Map retryPolicy**。真正差异更大的是 classic external ALB，而不是 “internal 有、external 没有”。
...
GCP Internal HTTP(S) Load Balancer 的重试策略是在 **URL Map 的 Route Rules 层级**配置的。这个机制与 Global/Regional External Application Load Balancer 在模型上是一致的，都是 `routeAction.retryPolicy`。
...
【默认行为】（未配置 retryPolicy 时）：
  • 不显式写 retryPolicy，GLB 仍可能对符合条件的请求自动补 1 次尝试
...
| 特性              | Global / Regional External ALB             | Internal HTTP(S) LB                        |
| ----------------- | ------------------------------------------ | ------------------------------------------ |
| **Retry Policy**  | 支持 URL Map retryPolicy                   | 支持 URL Map retryPolicy                   |
| **可配置性**      | 通过 routeRules 配置                       | 通过 routeRules 配置                       |
| **默认行为**      | 无 body 请求对 502/503/504 自动补 1 次尝试 | 无 body 请求对 502/503/504 自动补 1 次尝试 |
| **perTryTimeout** | 支持，最大 24 小时                         | 支持，最大 24 小时                         |
| **POST 重试**     | 默认不重试；显式策略也应避免               | 默认不重试；显式策略也应避免               |
| **重试目标**      | 健康后端                                   | 健康后端                                   |
...
- name: api-matcher
  ...
  - priority: 200
    ...
    routeAction:
      # 不配置 retryPolicy = 走默认行为（无 body 请求在 502/503/504 时可能补 1 次尝试）
      # POST 默认也不重试，双重保险
      timeout:
        seconds: 30
      weightedBackendServices:
...
| 特性         | 行为                                                  |
| ------------ | ----------------------------------------------------- |
| `numRetries` | 未显式设置；默认行为仍会对符合条件请求自动补 1 次尝试 |
...
| 维度           | 不配置（默认）                  | 显式配置                     |
| -------------- | ------------------------------- | ---------------------------- |
| **重试次数**   | 默认行为自动补 1 次尝试         | 显式控制（最多 25 次总尝试） |
...
如果你使用的是 **Global External Application Load Balancer** 而非 Internal LB，底层资源模型不同，但 retry 的主入口同样是 URL Map 的 `routeAction.retryPolicy`。
...
│  GCP GLB    │ ─────────────────────────────▶│  Nginx L7   │
│  (HTTPS)    │    retry: 默认对 GET 502/503/504 可重试1次 │  (反向代理)  │
│             │                               │             │
│ • 连接超时   │                               │ • proxy_    │
│ • 响应超时   │                               │   connect_  │
│ • 健康检查   │                               │   timeout   │
...
| 参数                           | 默认值 | 说明                                                 |
| ------------------------------ | ------ | ---------------------------------------------------- |
| `timeoutSec` (后端服务超时)    | 30s    | 等待后端响应的完整时间；生产上常见配置区间是 60-600s |
| `connectionDrainingTimeoutSec` | 300s   | 连接排空时间                                         |
...
**GLB 不是“完全不重试”，但默认行为非常克制。**
- 未配置 `retryPolicy` 时，**无 body 的请求**（典型是 `GET`）在遇到 `502` / `503` / `504` 时会默认再尝试 1 次
- `POST` 默认不会被重试
- 配置 `routeAction.retryPolicy` 后，GLB 才会按 URL map 规则执行更明确的 retry 条件和次数控制
- `numRetries` 表示总尝试次数，`1` 代表只发起第一次请求，不再追加重试

> **建议：GLB 层只做保守 retry，尤其适用于 GET / HEAD；复杂重试逻辑放在 Nginx/Kong 或应用层，并统一做幂等约束。**
...
| `proxy_next_upstream`         | http/server/location | `error timeout` | 默认就可能触发 upstream failover     |
...
**默认未配置时也有 retry/failover 行为**：

- 默认 `proxy_next_upstream error timeout;`
- 也就是说，Nginx 在连接 upstream、向 upstream 发送请求、读取 upstream 响应头时，如果发生 `error` 或 `timeout`，会尝试切换到下一个 upstream
- 这不是基于 HTTP 状态码的重试；如果你希望 `502` / `503` / `504` 也触发切换，需要显式配置
- 默认不会对已经发送到 upstream 的非幂等请求继续重试；只有加 `non_idempotent` 才会放开
- 如果 upstream 只有一个 healthy target，则即使触发条件满足，也没有第二个目标可切换
