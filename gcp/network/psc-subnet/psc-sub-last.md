...
- [**5 生产级多集群 IP 规划 (基于 192.168.64.0/20 架构)**](#5-生产级多集群-ip-规划-基于-19216864020-架构)
    - [**5.1 10 集群详细规划表 (Standard /20 Node Alignment)**](#51-10-集群详细规划表-standard-20-node-alignment)
  - [🔍 问题分析](#-问题分析)
  - [🛠 一、地址池基础演算](#-一地址池基础演算)
    - [1.1 固定池容量](#11-固定池容量)
    - [1.2 子网切割方案对比](#12-子网切割方案对比)
  - [🛠 二、严格按 /28 演算（最小子网方案）](#-二严格按-28-演算最小子网方案)
    - [2.1 /28 容量核算](#21-28-容量核算)
    - [2.2 PSC NAT Subnet 对于 /28 的实际使用](#22-psc-nat-subnet-对于-28-的实际使用)
    - [2.3 100个 /28 子网地址分配表（10集群 × 10 Attachments）](#23-100个-28-子网地址分配表10集群--10-attachments)
  - [🛠 三、完整 /28 子网精确计算表](#-三完整-28-子网精确计算表)
  - [🛠 四、PSC NAT Quota / Capacity 详细解释](#-四psc-nat-quota--abjacity-详细解释)
    - [4.1 PSC NAT 核心机制](#41-psc-nat-核心机制)
    - [4.2 关键 Quota 维度](#42-关键-quota-维度)
    - [4.3 NAT IP 容量计算公式](#43-nat-ip-容量计算公式)
    - [4.4 高访问量场景 NAT Subnet 扩展策略](#44-高访问量场景-nat-subnet-扩展策略)
  - [📊 五、架构流程图](#-五架构流程图)
  - [🛠 六、与现有规划的整合](#-六与现有规划的整合)
  - [⚠️ 七、注意事项与最佳实践](#️-七注意事项与最佳实践)
    - [7.1 子网选择建议](#71-子网选择建议)
    - [7.2 NAT Subnet 创建规范](#72-nat-subnet-创建规范)
    - [7.3 Service Attachment 绑定](#73-service-attachment-绑定)
    - [7.4 Quota 申请建议](#74-quota-申请建议)
    - [7.5 命名规范建议](#75-命名规范建议)
  - [总结](#总结)
- [27](#27)
  - [🔍 问题分析](#-问题分析-1)
  - [🛠 一、/27 基础容量核算](#-一27-基础容量核算)
    - [1.1 /27 单子网 IP 结构](#11-27-单子网-ip-结构)
  - [🛠 二、/27 完整子网分配表（100个，10集群 × 10 Attachments）](#-二27-完整子网分配表100个10集群--10-attachments)
    - [2.1 地址递增规律](#21-地址递增规律)
    - [2.2 逐集群详细分配](#22-逐集群详细分配)
  - [🛠 三、/27 vs /28 深度对比分析](#-三27-vs-28-深度对比分析)
    - [3.1 核心指标对比](#31-核心指标对比)
    - [3.2 场景选型决策树](#32-场景选型决策树)
    - [3.3 按业务类型的 Attachment 分级建议](#33-按业务类型的-attachment-分级建议)
    - [3.4 混合策略（最优实践）](#34-混合策略最优实践)
  - [📊 四、地址空间分布可视化](#-四地址空间分布可视化)
  - [⚠️ 五、/27 方案关键注意事项](#️-五27-方案关键注意事项)
    - [5.1 余量警告](#51-余量警告)
    - [5.2 Terraform 批量创建示例（/27）](#52-terraform-批量创建示例27)
    - [5.3 验证脚本](#53-验证脚本)
  - [总结对比](#总结对比)
- [quota](#quota)
- [PSC NAT IP 容量评估 \& 网络性能建模](#psc-nat-ip-容量评估--网络性能建模)
  - [1. 问题分析](#1-问题分析)
  - [2. 基础计算（你的问题）](#2-基础计算你的问题)
    - [并发连接能力](#并发连接能力)
...
我可以帮你做一份 生产级容量模型（abjacity planning）
...
| 扩展性   | 警告 | PSC 生产上更推荐“新增更大子网并挂到同一 Service Attachment”做平滑扩容；现有 subnet 也可以扩展 IPv4 range，但需要按生产变更方式执行。 |
...
## 11. Smooth Switch / Downtime：PSC NAT 子网扩容时会不会中断？

这一节专门回答你最关心的问题：

如果某个 attachment 当前使用的是：

```text
192.168.240.0/27
```

而且已经在生产中分配过 NAT IP、正在承载真实流量，后面你想把容量提升到：
...
从 GCP 官方能力来看，这件事是可行的，而且有两条路径：

1. 扩展现有 subnet 的 primary IPv4 range
2. 新建一个更大的 PSC subnet，并把它追加到同一个 service attachment

其中第二种更适合生产。

原因很简单：

- Google 官方明确说明：给 service attachment 增加额外 NAT subnets，可以在**不中断流量**的情况下完成
- Google 官方的 VPC subnet 文档也明确说明：扩展 subnet 的 primary IPv4 range 时，**子网内流量不中断**

所以你这个 `/27 -> /25` 的问题，工程上可以理解为：

- **支持扩容**
- **目标是无业务中断**
- **生产推荐路径是“加新池做平滑切换”，而不是把原地扩容当成唯一手段**
...
这种方式的优点是地址连续、规划整洁，不需要新建第二个 subnet。

但它的工程特点是：

- 属于直接修改现网资源
- 虽然通用 VPC 文档给了“流量不中断”的保证，但 PSC 文档没有单独展开解释“已有 NAT 会话如何重新分布”
- 不能像双池方案那样天然灰度和回退
- 如果 firewall、审计、白名单、观测规则只覆盖了旧范围，必须同步修正

因此它是一个**可用方案**，但不应是高价值 attachment 的默认首选。

### 方案 B：新增更大 subnet，挂到同一个 attachment
...
然后把二者都挂到同一个 service attachment。

这种方式更像真正的 `smooth switch`：

- 先保留旧池继续承载流量
- 把新池接入 attachment
- 观察新的 NAT IP 分配是否开始落到新池
- 等旧池水位降下来以后，再考虑是否移除旧池
...
- 已建立连接，预期不会因为你增加容量而被主动打断
- 新建连接，会在扩容完成后逐步开始使用新的地址空间

也就是说，扩容动作本质上是在增加可分配 NAT 地址池，而不是把旧连接“迁移”到新地址。
...
如果你的平台是 API 场景，前面是 Kong / Gateway，后面是 GKE/ILB，那么这类扩容通常不会直接造成业务 downtime。

真正更容易出问题的，往往不是扩容动作本身，而是配套配置没同步：

1. 新 CIDR 没加进 firewall
2. 后端来源校验只允许旧 NAT 段
3. 监控没覆盖新 subnet
4. 扩容动作做得太晚，attachment 在变更前就已经接近耗尽
...
大约是 **787 万理论并发 TCP 连接**。

所以 `/27 -> /25` 的意义不是“小幅优化”，而是：

- NAT IP 数从 `28` 增长到 `124`
- 理论连接容量提升大约 **4.4 倍**

如果你的平台里 attachment 后面挂的是高价值 API 入口，这个增幅是非常有意义的。
...
- 地址必须连续
- 你对现网 firewall / policy / 监控改造有把握
- 你能接受“可用但回退性较弱”的变更模型

### 规则 4：变更前先看 NAT 水位，不要只看 TPS

重点看：

- `private_service_connect/producer/used_nat_ip_addresses`
- consumer endpoint 数量
- propagated connection 数量
- backend 连接数

因为 NAT 扩容解决的是“地址池和连接池容量”，不是直接解决后端应用 TPS。

---

## 11.6 建议的平滑扩容步骤

如果要把一个 attachment 从 `/27` 级别提升到 `/25` 级别，推荐顺序如下：

1. 新建一个更大的 PSC subnet
2. 确认 backend firewall / 来源限制允许新 subnet
3. 把新 subnet 追加到现有 service attachment
4. 观察 NAT 地址使用和 attachment 连接状态
5. 等新连接稳定进入新池后，再决定是否保留旧 subnet
6. 如果需要简化规划，再安排后续维护窗口做收敛

这套流程的关键点是：

- attachment 本身不换
- consumer endpoint 不换
- 流量路径不换
- 你只是在 producer 侧增加地址池

因此它最接近你要的 `smooth switch`。

---

## 11.7 一句话总结

如果一个 PSC attachment 已经在使用 `192.168.240.0/27`，后面想扩到 `/25`，从 GCP 能力上这是可行的，而且目标是无中断。

但从生产可控性、回滚能力、观测清晰度来看，**最推荐的方式不是直接原地改 CIDR，而是新建一个更大的 PSC subnet，挂到同一个 service attachment 上，做双池平滑扩容**。
