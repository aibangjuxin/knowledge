# Q 
**在 GKE Deployment 中，一个 Pod 内包含多个容器（Init Container + Java 主容器），每个容器究竟占用多少 CPU / Memory？如何系统化分析资源使用情况？**
...
# 1. 问题分析
你现在关注的问题是：  
**如果使用 Sidecar来注入 JDK 工具进入原有 Pod，会不会影响 Pod 的内存占用？尤其是在你要分析 Java 容器的真实内存使用时，会不会扭曲结果？**
