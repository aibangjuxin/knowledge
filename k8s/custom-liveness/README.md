...
- **外部资源访问验证**：通过 API 接口检测 Pod 是否能访问外部资源（如www.baidu.com）
...
graph TB
    subgraph K8S["Kubernetes Cluster"]
        ...
        subgraph PROXY["Proxy Failover System"]
    ...
    subgraph HEALTH["Health Check Flow"]
        J["Kubernetes Probe"] --> B
        B --> K{"Test External Connectivity"}
...
sequenceDiagram
    ...
    K->>CM: 1. 创建ConfigMap存储health-check.py
    K->>P: 2. 创建Pod引用ConfigMap
    P->>C: 3. 启动Container
...
stateDiagram-v2
    ...
    note right of PrimaryActive: 客户端使用主代理 http://squid-proxy-primary:3128
    note right of SecondaryActive: 客户端切换到备用代理 http://squid-proxy-secondary:3128
    note right of AllFailed: 返回503错误 等待代理恢复
```
