...
|触发条件|CPU利用率、内存、Prometheus自定义指标等|历史资源使用情况（自动推荐/调整）|
...
当 VPA 检测到资源不足，会自动重启 Pod 并应用新的 resource requests/limits 那么这个过程如何确保服务高可用.比如我们生产环境都是默认一个Deployment 至少2个Pod.
...
| Auto    | 自动删除并重建 Pod 以调整资源。⚠️生产中应与 HPA、PDB 配合使用。 |
