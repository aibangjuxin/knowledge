...
> 目标：确认 GKE Pod 到 Cloud DNS、DNS Peering、Forwarding Zone、NodeLocal DNSCache、Java DNS 缓存和 `ndots` 行为，形成一个更适合生产排查和落地验证的版本。
>
...
> **重要**：生产排查中不要把 `169.254.254.254` 当作 GKE 官方验证入口。Google 文档对 Cloud DNS for GKE 的验证路径使用 `169.254.169.254`，NodeLocal 使用 `169.254.20.10`。
...
| 原文点 | 判断 | 修正建议 |
|---|---|---|
| "GKE 1.36+ 全面使用 CoreDNS，并默认启用 NodeLocal DNSCache" | 不够准确 | GKE 1.36+ 的 kube-dns 底层实现变为 CoreDNS；Autopilot 默认 Cloud DNS；Standard 默认 DNS provider 仍需要区分 kube-dns 与 Cloud DNS for GKE。NodeLocal 在 Autopilot 默认启用，Standard 从 `1.34.1-gke.3720000` 起默认启用。实际集群建议通过 `kubectl get pods -n kube-system` 确认，不要仅凭集群版本号推断。 |
...
| `api.aliyun.cloud.region.aibang` 有 5 个 dots | 错误 | 这个名字有 4 个普通分隔点。默认 `ndots:5` 会先追加 search path，再查询原始名字。 |
...
graph TD
  ...
  C --> F{"缓存命中"}
  F -->|"是"| G["返回结果"]
  F -->|"否，cluster.local"| I["kube-dns 权威解析"]
  F -->|"否，外部域名"| D
  E --> I
  I -->|"外部查询上游"| D
...
> **注意**：NodeLocal 对 `cluster.local` 的 miss 转发到 kube-dns（kube-dns 本身是权威）；只有外部域名才继续走 Cloud DNS / VPC DNS。两条路径不要混淆。
...
graph TD
  ...
  B -->|"未命中"| C["Cluster-scoped private / forwarding zone"]
  C -->|"未命中"| D["VPC outbound server policy"]
  D -->|"未命中"| E["VPC response policy"]
  E -->|"未命中"| F["VPC managed private zones longest suffix match"]
  F --> G{"命中的 zone 类型"}
  G -->|"Private zone"| H["返回 zone 内记录或 NXDOMAIN"]
  G -->|"Forwarding zone"| I["转发到 target DNS（源 35.199.192.0/19）"]
  G -->|"Peering zone"| J["切到 producer VPC 重新执行解析顺序"]
  F -->|"所有 zone 均未命中"| K["Compute Engine internal DNS"]
  K -->|"未命中"| L["Public DNS recursion"]
```
...
- `F --> K` 这条路径只在**所有 VPC managed private zones 都未命中**时才触发，不是并列关系。
...
graph TD
  ...
  Z2 -->|"切换到 producer VPC"| Z1["VPC1 name resolution order"]
  Z1 --> F1["VPC1 Forwarding Zone: aibang."]
  F1 -->|"源 35.199.192.0/19"| D1["Enterprise DNS targets"]
...
```mermaid
graph TD
  A["应用提交查询名"] --> B{"是否有尾部 root 点？"}
  B -->|"有（如 aibang.）"| C["绝对 FQDN，直接查询，不走 search"]
  B -->|"无"| D["计算普通分隔点数量"]
  D --> E{"点数 >= ndots（5）？"}
  E -->|"是"| F["先查原始名字，再决定是否继续 search"]
  E -->|"否（如 4 点）"| G["先追加 search path，全部失败再查原始名"]
  G --> H["产生 N+1 次 DNS 查询（N = search list 长度）"]
```

### 6.4 对 `api.aliyun.cloud.region.aibang` 的影响
...
### 6.5 为什么会看到 `aibang.cluster.local`
...
/**
 * 在 DNS lookup 层追加尾部点，使查询走绝对 FQDN，跳过 Kubernetes search path。
 * 注意：需验证目标 JVM 版本对尾部点 hostname 的兼容性，
 *       旧版 JVM / Android 环境可能对 hostname. 尾部点处理有差异。
 */
...
> **不要**把 `169.254.0.0/16` 当作 forwarding target 入站源范围。
...
```text
service-a.internal.aibang.   # 使用尾部点（仅 DNS lookup 层）
networkaddress.cache.ttl=30
networkaddress.cache.negative.ttl=5
...
- 不要在 HTTP URL 中直接写尾部点，未经 Java HTTP client / TLS / Gateway Host 兼容性验证。
...
*Document version: gpt5-6 — 2026-05-24*
