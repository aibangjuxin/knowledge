...
YAML (Fail): gcloud 的 YAML 导出格式是为了 API 交互设计的复杂结构，包含 kind, etag 等元数据。直接将导出的 YAML 用于导入时，因为结构繁琐且严格（缩进敏感），很容易出错，且难以进行简单的文本编辑。
TXT (Success - BIND): .txt 文件实际上是 BIND Zone File (RFC 1035 标准)。这是 DNS 领域的通用语言，结构极其扁平（每行一条记录），非常适合机器读取和文本处理工具（如 awk, sed）操作。
2. 冲突解决策略 (最关键的原因)

原来的问题:
新创建的 Zone 会自动由 GCP 分配一套默认的 SOA 和 NS 记录。
之前的逻辑尝试导入所有源记录（包括源 Zone 的 SOA/NS）。这会导致与新 Zone 的默认记录发生冲突，或者因为权限问题无法覆盖系统管理的记录 而报错。
现在的方案 (Filter + Bulk Import):
我们利用 BIND 格式的便利性，使用 awk 精准剔除了导出文件中的 SOA 和根域名 NS 记录。
使用 --delete-all-existing 标志：这会清空新 Zone 中任何可能存在的用户记录（保持干净），但保留了 GCP 自动生成的系统记录（因为导入文件中没有这些记录去覆盖它们）。
结果：用户自定义的记录（A, CNAME 等）被完美导入，而系统管理的记录保持了新环境的正确配置。
一句话总结： 成功的关键在于利用 BIND 标准格式 的易处理性，通过脚本 自动过滤掉了与新环境冲突的系统记录 (NS/SOA)，从而实现了无缝的批量迁移。
