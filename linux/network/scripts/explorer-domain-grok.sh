...
# Domain Explorer Script - Grok Optimized Version
# Usage: ./explorer-domain-grok.sh <domain>
# Example: ./explorer-domain-grok.sh www.baidu.com
# Optimizations: Enhanced parallel execution, improved caching, better error handling, fixed bugs, consistent timeouts
...
OPTIONAL_CMDS=("nslookup" "openssl" "nmap" "nc")
...
for cmd in "${OPTIONAL_CMDS[@]}"; do
    if ! command_exists "$cmd"; then
        print_warning "Optional command missing: $cmd - Some features may be unavailable"
    fi
done
...
fast_web_analysis() {
    ...
    if command_exists curl; then
        ...
        if [ -s "$TEMP_DIR/https_headers" ]; then
            ...
            grep -iq "strict-transport-security" "$TEMP_DIR/https_headers" && print_success "HSTS enabled" || print_warning "HSTS not found"
            grep -iq "x-frame-options" "$TEMP_DIR/https_headers" && print_success "X-Frame-Options set" || print_warning "X-Frame-Options not found"
            grep -iq "content-security-policy" "$TEMP_DIR/https_headers" && print_success "CSP enabled" || print_warning "CSP not found"
        else
...
fast_port_scan() {
    ...
    if command_exists nmap; then
        ...
        run_command_fast "nmap -p $common_ports --open -T4 $ip" "Common ports scan" 15 "port_scan_common"
        
        print_subheader "Service Detection (Top Ports)"
        # Quick service detection on open ports only
        run_command_fast "nmap -sV -T4 --version-intensity 2 -p $common_ports $ip" "Service detection" 20 "port_scan_service"
...
fast_subdomain_discovery() {
    ...
    # Display results
    if [ -f "$TEMP_DIR/found_subdomains" ] && [ -s "$TEMP_DIR/found_subdomains" ]; then
...
fast_network_info() {
    ...
    if [ -f "$TEMP_DIR/dns_propagation" ] && [ -s "$TEMP_DIR/dns_propagation" ]; then
        ...
        print_warning "No DNS propagation results available"
    fi
...
print_header "DOMAIN EXPLORATION REPORT (GROK OPTIMIZED)"
...
echo "✓ Fixed security analysis waiting mechanism"
echo "✓ Added caching for port scans"
echo "✓ Improved empty file handling"
echo "✓ Separated required and optional commands checks"
