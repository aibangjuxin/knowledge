...
- Firefox 即使在设置了 `SameSite=None` 和 `credentials: true` 的情况下也会阻止这些 Cookie。随着现代浏览器对第三方 Cookie 的限制越来越严格，这个问题将愈发突出。==> 这个说法就要进行验证，或者不严谨。

为了避免依赖 `SameSite=None + Secure` 这种需要严格配置且在不同浏览器下行为不一致的方案，我们采用**域名统一（same-site）**来从根本上规避第三方 Cookie 的兼容性问题。
