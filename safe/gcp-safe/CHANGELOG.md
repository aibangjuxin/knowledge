...
## v2.0.2 (2025-11-10) - 权限优化

### ✨ 改进

**优化权限要求：使用 `list` 替代 `describe` 命令**

#### 变更说明
- `check_keyring()` 现在使用 `gcloud kms keyrings list` 而不是 `describe`
- `check_crypto_key()` 现在使用 `gcloud kms keys list` 而不是 `describe`
...
#### 优势
1. **更符合最小权限原则** - `list` 权限通常比 `get` 权限更容易获得
2. **更灵活** - 适用于只有 `list` 权限但没有 `describe` 权限的场景
3. **功能完整** - 仍然可以获取密钥的用途和状态信息

### 📚 新增工具
- `test-permissions.sh` - 测试 describe vs list 权限的差异
